...
- Repeatable `--index KIND` selects kinds to build: `ngram` (optional `--width` / `--norm` after it) and `ast` (tree-sitter node-kind index over rust/python/javascript/typescript/tsx/go/java; opt-in, powers upcoming structural search). Default catalog: identity and ascii-lower trigram.
