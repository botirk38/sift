...
/// Scratch for one PCRE2 search. Create it with [`Regex::create_match_data`]
/// and pass it to [`Regex::is_match_with`] / [`Regex::find_at_with`] so the
/// search does not pull from the internal match-data pool.
...
impl MatchData {
    ...
    /// Returns the start and end positions of the Nth capture group from the
    /// last search.
    ///
