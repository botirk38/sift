...
use crate::{
    error::Error,
    ffi::{Code, CompileContext, MatchConfig},
    pool::{Pool, PoolGuard},
};

pub use crate::ffi::MatchData;
...
impl Regex {
    ...
    /// Like [`Regex::is_match`], but uses caller-owned match data instead of
    /// the internal pool.
    pub fn is_match_with(
        ...
        self.is_match_at_with(match_data, subject, 0)
    }
...
impl Regex {
    ...
    /// Like [`Regex::is_match_at`], but uses caller-owned match data instead
    ...
    /// Like [`Regex::find_at`], but uses caller-owned match data instead of
    /// the internal pool.
    pub fn find_at_with<'s>(
        &self,
        match_data: &mut MatchData,
        subject: &'s [u8],
        start: usize,
...
impl Regex {
    ...
    /// Returns match data for use with [`Regex::is_match_with`] and
    /// [`Regex::find_at_with`]. Reuse it across searches of this regex.
