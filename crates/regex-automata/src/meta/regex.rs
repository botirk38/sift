...
pub struct Regex {
    ...
    ///
    /// When [`Config::pool`] is `false`, this is `None` and the higher level
    /// convenience APIs that pull from the pool must not be used. Callers
    /// should use the `*_with` APIs instead.
...
impl Regex {
    ...
    /// This is like [`Regex::is_match`], but requires the caller to
    /// explicitly pass a [`Cache`].
    ///
    /// Passing a `Cache` explicitly bypasses the internal memory pool used by
    /// `is_match`. That pool can be slower when a `Regex` is used from multiple
    /// threads simultaneously on small haystacks.
    ///
    /// This uses the same earliest-match semantics as [`Regex::is_match`].
    ///
    /// # Example
    ///
    /// ```
    ...
    ///
    /// let re = Regex::builder()
    ///     .configure(Regex::config().pool(false))
    ///     .build(r"foo[0-9]+")?;
    /// let mut cache = re.create_cache();
    /// assert!(re.is_match_with(&mut cache, "foo12345"));
    /// assert!(!re.is_match_with(&mut cache, "foobar"));
    ///
    /// # Ok::<(), Box<dyn std::error::Error>>(())
    /// ```
    #[inline(always)]
    pub fn is_match_with<'h, I: Into<Input<'h>>>(
        &self,
        cache: &mut Cache,
        input: I,
    ) -> bool {
        ...
        self.imp.strat.is_match(cache, &input)
    }
...
impl Regex {
    ...
    fn cache_pool(&self) -> &CachePool {
        self.pool.as_ref().expect(
            "Regex built with Config::pool(false); use is_match_with / search_with",
        )
    }
    ...
    fn new_pool(
        ...
        Some(Pool::with_capacity(config.get_pool_capacity(), create))
    }
}
...
impl Clone for Regex {
    fn clone(&self) -> Regex {
        ...
        let pool = Regex::new_pool(imp.info.config(), &imp.strat);
        Regex { imp, pool }
    }
}
...
impl Config {
    ...
    /// Whether to construct the internal [`Cache`] pool used by convenience
    /// APIs such as [`Regex::is_match`].
    ///
    /// This is enabled by default. Disable it when every search will pass an
    /// explicit [`Cache`] to the `*_with` APIs. A `Regex` built with the pool
    /// disabled must not use the pooled convenience APIs.
    pub fn pool(self, yes: bool) -> Config {
        Config { pool: Some(yes), ..self }
    }
    ...
    /// Returns whether the internal cache pool is enabled, as set by
    /// [`Config::pool`].
    ///
    /// If it was not explicitly set, then this returns `true`.
    pub fn get_pool(&self) -> bool {
        self.pool.unwrap_or(true)
    }
...
impl Builder {
    ...
    pub fn build_many_from_hir<H: Borrow<Hir>>(
        ...
        let pool = Regex::new_pool(&self.config, &strat);
