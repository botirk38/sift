...
- Prefer enums over bools for real alternatives with distinct cases (`MatchEmissionMode`, `ZeroCounts`, `SearchMode`). Do not model on/off switch state as `Off`/`On` entity enums — use `bool` (do not extend the existing `Quiet` / `InvertMatch` Off/On pattern to new flags).
